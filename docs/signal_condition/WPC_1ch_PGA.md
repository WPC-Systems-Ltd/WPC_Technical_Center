...
*   **差動訊號輸入**：採用差動輸入架構，具備高達 140dB 的 CMRR，能有效抑制共模雜訊干擾。
...
*   **輸入電壓範圍**：±10 V (差動 Differential)
*   **輸出電壓範圍**：±10 V (單端 Single-ended)
*   **頻寬 (Bandwidth)**：9 KHz
*   **共模抑制比 (CMRR)**：> 140 dB
...
| 6        | D0            | 增益控制位元 0                    |
| 7        | GND           | 數位/訊號接地                     |
| 8        | Output        | 單端訊號輸出 (Single-ended)       |
| 24V+     | 24V           | 電源輸入 正極                     |
| 24V-     | 0V            | 電源輸入 負極                     |

:::warning
D0~D3 僅支援 3.3V / 5V，切勿接 24V，否則會損壞模組。
:::
...
下圖展示了 WPC 1ch PGA 在系統中的典型連線範例。本模組接受差動訊號輸入，可有效抑制長距離傳輸或工業環境中的共模雜訊，並將放大後的訊號以單端形式輸出至後端 DAQ 系統。
