---
title: Product Specifications
sidebar_position: 5
description: "| Item | Specification | | :------------------------ | :-------------------- | | Drone Size (Wheelbase) | 550mm | | Total Length | 75cm | | Total Height | 26..."
keywords: [Product Specifications, WPC drone, flight controller, UAV tutorial, MIT drone, pre-flight checklist]
---
## Product Specifications

| Item                      | Specification         |
| :------------------------ | :-------------------- |
| **Drone Size (Wheelbase)** | 550mm                 |
| **Total Length**          | 75cm                  |
| **Total Height**          | 26.5cm                |
| **Net Weight**            | 2.6kg                 |
| **Propulsion System**      | Battery & Motor       |
| **Max Payload**           | 2.5kg                 |
| **Max Takeoff Weight**    | 6kg                   |
| **Max Wind Resistance**   | 17.2m/s               |
| **Hovering Accuracy**     | 10cm                  |
| **Flight Time**           | 10~20min              |
| **Operating Range**       | 3.4km                 |
| **RC Interface**          | PWM                   |
| **SDK Interface**         | UART                  |
| **Takeoff Method**        | VTOL                  |
| **Landing Method**        | VTOL                  |
| **Sensor Fusion Rate**    | 400Hz                 |
| **Attitude Control Rate** | 400Hz                 |
| **Flight Recorder**       | USB                   |
| **Frame Material**        | Carbon Fiber          |
